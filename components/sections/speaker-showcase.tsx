...
const speakingIcons = [Mic, Users, Globe, Award];
...
export function SpeakerShowcase({ locale, dict }: SpeakerShowcaseProps) {
  const highlights = dict.speakerShowcase.highlights;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <BlurFade delay={0.2}>
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
              {dict.speakerShowcase.title}
            </h2>
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
              {dict.speakerShowcase.subtitle}
            </p>
          </div>
        </BlurFade>
          ...
          <BlurFade delay={0.6}>
            <div className="space-y-6">
              {highlights.map((highlight, index) => {
                const Icon = speakingIcons[index];
                return (
              ...
              <div className="pt-6">
                <Button asChild size="lg">
                  <LocalizedLink route="speaking" locale={locale as Locale} className="px-6 py-2">
                    {dict.speakerShowcase.buttonText}
                  </LocalizedLink>
                </Button>
              </div>
            </div>
        ...
        <BlurFade delay={0.8}>
          <div className="bg-background rounded-2xl shadow-lg p-8">
            <h3 className="text-xl font-bold text-foreground mb-6">{dict.speakerShowcase.recentTitle}</h3>
